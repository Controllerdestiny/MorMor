...
public class Music_163
{
    ...
    public async Task<string> GetMusicUrlByid(int id)
    {
        try
        {
            var urls = await GetMusicUrlByid(new int[] { id });
    ...
    /// <param name="id"></param>
    /// <returns></returns>
    public async Task<Dictionary<int, string>> GetMusicUrlByid(IEnumerable<int> ids)
    {
        var param = new
        {
            ids = $"[{string.Join(",", ids)}]",
    ...
    /// <returns></returns>
    /// <exception cref="Exception"></exception>
    public async Task<MusicData> GetMusicByID(int id)
    {
        try
        {
            var list = await GetMusicByID(new int[] { id });
            if (list.Count == 0)
            {
    ...
    /// <returns></returns>
    /// <exception cref="Exception"></exception>
    public async Task<List<MusicData>> GetMusicByID(IEnumerable<int> ids)
    {
        ...
        if (data.Value<int>("code") == 200)
        {
            ...
            for (int i = 0; i < musicList.Count; i++)
            {
                ...
                var musicID = musicList[i].Value<int>("id")!;
